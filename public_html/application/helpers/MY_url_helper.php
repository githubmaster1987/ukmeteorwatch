...
/*
    $query = $CI->db->query("SELECT `id` FROM `urls` WHERE `url`= ? LIMIT 1", array($url));
    ...
    if ($query->num_rows() > 0) {
       $id  = $query->row()->id;
        ...
        $CI->db->insert('urls', array('url' => $url));
        $id = $CI->db->insert_id();
    }
*/
    // now convert the ID into its base36 instance and return
    // base_convert($id, 10, 36)

    if (! isset($CI->urls)){
        $urls = array();
        $db_urls = $CI->db->query("SELECT * FROM urls")->result_array();
        foreach ($db_urls as $db_url){
            $urls["{$db_url['url']}"] = $db_url['id'];
        }

        $CI->urls = $urls;
    }

    if (isset($CI->urls["{$url}"])){
        $id = $CI->urls["{$url}"];
    }
    else{
        $CI->db->insert('urls', array('url' => $url));
...
function get_short_url($url_code_id)
{
    return '/link/go/'.base_convert($url_code_id, 10, 36);
