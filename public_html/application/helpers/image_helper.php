...
if (!function_exists('profile_img'))
{
	...
	function profile_img($id, $img, $preset)
	{
		...
		{
			return img('media/profile/'.$id.'/'.$pathinfo['filename'].'-'.$preset.'.'.$pathinfo["extension"]);
	...
	function tmp_profile_img($id, $img, $preset)
	{
		// load the allowed image presets
		$sizes = config_item("image_sizes");
		...
		if (isset($sizes[$preset]))
		{
			return img('media/profile/tmp/'.$pathinfo['filename'].'-'.$preset.'.'.$pathinfo["extension"]);
			//$new_path = $pathinfo["dirname"] . "/" . $pathinfo["filename"] . "-" . implode("x", $sizes[$preset]) . "." . $pathinfo["extension"];
		}
	...
	function tmp_img($img, $preset)
	{
		...
		if (isset($sizes[$preset]))
		{
			return img('media/tmp/'.$pathinfo['filename'].'-'.$preset.'.'.$pathinfo["extension"]);
		}

		return FALSE;
	}

	function blog_img($img, $preset)
	{
		// load the allowed image presets
		$sizes = config_item("image_sizes");
		...
		if (isset($sizes[$preset]))
		{
			return img('media/blog/'.$pathinfo['filename'].'-'.$preset.'.'.$pathinfo["extension"]);
		}
...
function generate_thumb($image='', $width=0, $height=0, $cropratio = FALSE, $color=NULL, $nocache = FALSE, $quality = 100)
{
	if (empty($image))
	{
	...
	define('CACHE_DIR',				FCPATH . '/uploads/_cache/');
	define('DOCUMENT_ROOT',			FCPATH);
	...
	$maxWidth		= (!empty($size[0])) ? (int) $width : 0;
	$maxHeight		= (!empty($size[1])) ? (int) $height : 0;
	...
	$quality	= (!empty($quality)) ? (int) $quality : DEFAULT_QUALITY;
	...
	if (!$nocache && file_exists($resized))
	{
